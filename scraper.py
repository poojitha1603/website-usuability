...
def check_broken_links(html, base_url):
    ...
    for link in links[:15]:  # limit to 15 links to avoid timeout
        href = link["href"]
        ...
        if not full_url.startswith("http"):
            continue
        try:
            r = requests.head(full_url, timeout=5, headers=headers, allow_redirects=True)
...
def check_mobile_responsiveness(html):
    ...
    # Fixed width elements (bad for mobile)
